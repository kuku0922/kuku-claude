...
class WeChatPublisher:
    ...
    def load_credentials(self):
        ...
        # 检查是否为占位符
        if not appid or appid == 'your-wechat-appid-here':
            raise ValueError(
                f"请配置微信公众号 AppID:\n"
                f"  配置文件: {config_path}\n"
                f"  设置 wechat.appid 字段"
            )

        if not appsecret or appsecret == 'your-wechat-appsecret-here':
            raise ValueError(
                f"请配置微信公众号 AppSecret:\n"
                f"  配置文件: {config_path}\n"
                f"  设置 wechat.appsecret 字段"
            )
